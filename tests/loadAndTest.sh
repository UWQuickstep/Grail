...
vertex_load_sql="/Grail/datagen/"$algo"/create_and_load_vertex.sql"
psql travis_ci_test -c "\i "$vertex_load_sql

edge_load_sql="/Grail/datagen/"$algo"/create_and_load_edge.sql"
psql travis_ci_test -c "\i "$edge_load_sql

#executes generated algorithmic sql and loads into next table of the db
algo_sql="/Grail/analytics/"$algo".sql"
psql travis_ci_test -c "\i "$algo_sql

#exports tuples from next table as flat file
psql travis_ci_test -t -c "select * from next order by id" > /Grail/output/$algo.out

#compares the diff between golden file and exported file
python /Grail/compareoutput.py -f "/Grail/output/"$algo"_golden.out" -F "/Grail/output/"$algo".out"
ret=$?
if [ $ret -ne 0 ]; then
